...
// ANSI codes, used only when Options.Color is set. The caller decides; this
...
func renderTable(w io.Writer, r Report, o Options) error {
	c := painter(o.Color)
...
func nodeTable(w io.Writer, r Report, o Options, c colorer) error {
	if len(r.Nodes) == 0 {
...
func detail(n NodeRow, verbose bool) string {
	var parts []string
...
func findingTable(w io.Writer, r Report, c colorer) error {
	if len(r.Findings) == 0 {
		return nil
	}
	fmt.Fprintf(w, "%s\n", c(bold, "FINDINGS"))
...
func ledgerTable(w io.Writer, r Report, c colorer) error {
...
func summary(w io.Writer, r Report, c colorer) error {
