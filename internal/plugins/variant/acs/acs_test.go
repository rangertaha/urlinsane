...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := acs.Spec(varianttest.Boards())
	if s.ID != "acs" {
		t.Errorf("ID = %q, want %q", s.ID, "acs")
	}
	if s.Title == "" {
...
// a key adjacent to one typed instead of it.
func TestGeneratesKnownVariants(t *testing.T) {
	got := acs.Spec(varianttest.Boards()).Gen("abc")
	want := []string{"sbc", "qbc", "zbc"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"abc", "example", "example.com"} {
		for _, g := range acs.Spec(varianttest.Boards()).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range acs.Spec(varianttest.Boards()).Gen("abc") {
		if seen[g] {
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := acs.Spec(varianttest.Boards()).Gen(""); len(got) != 0 {
