...
// every separator normalised to each of the others.
func TestGeneratesKnownVariants(t *testing.T) {
	got := sep.Spec().Gen("a-b_c")
	want := []string{"a-b-c", "a_b_c", "a.b.c", "abc"}
	if miss := varianttest.Missing(got, want); len(miss) > 0 {
		t.Errorf("Gen(%q) missing %q\ngot %q", "a-b_c", miss, got)
	}
}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"a-b_c", "example", "example.com"} {
		for _, g := range sep.Spec().Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range sep.Spec().Gen("a-b_c") {
		if seen[g] {
			t.Errorf("Gen(%q) returned %q more than once", "a-b_c", g)
		}
		seen[g] = true
	}
}
