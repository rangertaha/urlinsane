...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := gi.Spec(varianttest.Langs())
	if s.ID != "gi" {
		t.Errorf("ID = %q, want %q", s.ID, "gi")
	}
	if s.Title == "" {
...
// a grapheme of the language inserted at each gap.
func TestGeneratesKnownVariants(t *testing.T) {
	got := gi.Spec(varianttest.Langs()).Gen("ab")
	want := []string{"aab", "cab", "abc"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"ab", "example", "example.com"} {
		for _, g := range gi.Spec(varianttest.Langs()).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range gi.Spec(varianttest.Langs()).Gen("ab") {
		if seen[g] {
			t.Errorf("Gen(%q) returned %q more than once", "ab", g)
		}
		seen[g] = true
	}
}
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := gi.Spec(varianttest.Langs()).Gen(""); len(got) != 0 {
