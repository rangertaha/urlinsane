...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := gr.Spec(varianttest.Langs())
	if s.ID != "gr" {
		t.Errorf("ID = %q, want %q", s.ID, "gr")
	}
	if s.Title == "" {
...
// each character replaced by another grapheme.
func TestGeneratesKnownVariants(t *testing.T) {
	got := gr.Spec(varianttest.Langs()).Gen("abc")
	want := []string{"bbc", "cbc", "aac"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"abc", "example", "example.com"} {
		for _, g := range gr.Spec(varianttest.Langs()).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range gr.Spec(varianttest.Langs()).Gen("abc") {
		if seen[g] {
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := gr.Spec(varianttest.Langs()).Gen(""); len(got) != 0 {
