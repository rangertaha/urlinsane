...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := tld.Spec([]string{"com", "net", "co.uk"})
	if s.ID != "tld" {
		t.Errorf("ID = %q, want %q", s.ID, "tld")
	}
	if s.Title == "" {
...
// the public suffix replaced by every other known suffix.
func TestGeneratesKnownVariants(t *testing.T) {
	got := tld.Spec([]string{"com", "net", "co.uk"}).Gen("example.com")
	want := []string{"example.net", "example.co.uk"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"example.com", "example", "example.com"} {
		for _, g := range tld.Spec([]string{"com", "net", "co.uk"}).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range tld.Spec([]string{"com", "net", "co.uk"}).Gen("example.com") {
		if seen[g] {
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := tld.Spec([]string{"com", "net", "co.uk"}).Gen(""); len(got) != 0 {
