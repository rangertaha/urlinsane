...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := hs.Spec(varianttest.Langs())
	if s.ID != "hs" {
		t.Errorf("ID = %q, want %q", s.ID, "hs")
	}
	if s.Title == "" {
...
// a word replaced by one that sounds like it.
func TestGeneratesKnownVariants(t *testing.T) {
	got := hs.Spec(varianttest.Langs()).Gen("to")
	want := []string{"two", "too"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"to", "example", "example.com"} {
		for _, g := range hs.Spec(varianttest.Langs()).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range hs.Spec(varianttest.Langs()).Gen("to") {
		if seen[g] {
			t.Errorf("Gen(%q) returned %q more than once", "to", g)
		}
		seen[g] = true
	}
}
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := hs.Spec(varianttest.Langs()).Gen(""); len(got) != 0 {
