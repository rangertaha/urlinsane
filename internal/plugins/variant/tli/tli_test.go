...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := tli.Spec([]string{"com", "net", "co.uk"})
	if s.ID != "tli" {
		t.Errorf("ID = %q, want %q", s.ID, "tli")
	}
	if s.Title == "" {
...
// a second suffix appended after the real one.
func TestGeneratesKnownVariants(t *testing.T) {
	got := tli.Spec([]string{"com", "net", "co.uk"}).Gen("example.com")
	want := []string{"example.com.net", "example.com.co.uk"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"example.com", "example", "example.com"} {
		for _, g := range tli.Spec([]string{"com", "net", "co.uk"}).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range tli.Spec([]string{"com", "net", "co.uk"}).Gen("example.com") {
		if seen[g] {
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := tli.Spec([]string{"com", "net", "co.uk"}).Gen(""); len(got) != 0 {
