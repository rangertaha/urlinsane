...
// each adjacent pair transposed.
func TestGeneratesKnownVariants(t *testing.T) {
	got := cs.Spec().Gen("abcd")
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"abcd", "example", "example.com"} {
		for _, g := range cs.Spec().Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range cs.Spec().Gen("abcd") {
		if seen[g] {
			t.Errorf("Gen(%q) returned %q more than once", "abcd", g)
		}
		seen[g] = true
	}
}
