...
func testRegistry(t *testing.T) *graph.Registry {
	t.Helper()
	r := graph.NewRegistry()
	for _, name := range variant.NameableTypes {
	...
	if _, err := r.AddRel(variant.RelDef()); err != nil {
		t.Fatalf("rel %s: %v", variant.Rel, err)
	}
...
func TestGeneratesVariants(t *testing.T) {
	_, _, d, o := run(t, only(t, "co"), variant.TypeDomain, "example.com")
...
func TestSuffixIsPreserved(t *testing.T) {
	_, _, d, _ := run(t, only(t, "co"), variant.TypeDomain, "www.example.co.uk")
...
func TestEveryOperatorEmitsVariantRel(t *testing.T) {
	...
	for _, op := range ops {
		...
		if !found {
			t.Errorf("%s does not declare %s, so the engine would not treat it as a variant operator", op.Id(), variant.Rel)
		}
		if len(e.Props) != 2 {
...
func TestBindsByCapability(t *testing.T) {
	...
	for _, tc := range []struct{ typ, key string }{
		{variant.TypeDomain, "example.com"},
		{variant.TypeUsername, "example"},
		{variant.TypePackage, "example"},
		{variant.TypeRepo, "acme/example"},
		{variant.TypeEmail, "bob@example.com"},
	} {
...
func TestEmailVariesLocalPartOnly(t *testing.T) {
	_, _, d, _ := run(t, only(t, "co"), variant.TypeEmail, "bob@example.com")
	for _, k := range variantKeys(d) {
...
func TestDomainOnlyAlgorithmsNarrowTheirSelector(t *testing.T) {
	for _, id := range []string{"si", "tld"} {
		...
		if len(tr.On.Types) != 1 || tr.On.Types[0] != variant.TypeDomain {
...
func TestTLDSwapChangesOnlyTheSuffix(t *testing.T) {
	_, _, d, _ := run(t, only(t, "tld"), variant.TypeDomain, "example.com")
	keys := variantKeys(d)
...
func TestSubdomainInsertionPrependsLabels(t *testing.T) {
	_, _, d, _ := run(t, only(t, "si"), variant.TypeDomain, "example.com")
	keys := variantKeys(d)
...
func TestNothingToSayIsEmptyNotFailed(t *testing.T) {
	_, _, d, o := run(t, only(t, "ho"), variant.TypeDomain, "example.com")
...
func TestNamespaceConfusion(t *testing.T) {
	_, _, d, _ := run(t, only(t, "nsc"), variant.TypePackage, "@acme/tool")
	keys := variantKeys(d)
...
// testLang is a variant.Language a test can write down. The real one queries the
// dataset; an operator that could only be exercised against a database would
// not be the pure function §4.3 says it is.
type testLang struct{}

func (testLang) Code() string                    { return "xx" }
func (testLang) Name() string                    { return "Test" }
func (testLang) Vowels() []string                { return []string{"a", "e", "i", "o", "u"} }
func (testLang) Graphemes() []string             { return []string{"gh", "oo"} }
func (testLang) Numerals() map[string][]string   { return map[string][]string{"1": {"one"}} }
func (testLang) Homoglyphs() map[string][]string { return map[string][]string{"o": {"0"}} }
func (testLang) Homophones() [][]string          { return [][]string{{"goo", "gu"}} }
func (testLang) Misspellings() [][]string        { return [][]string{{"googel", "google"}} }

// Keyboard algorithms need no language: variant.Adjacency comes from pkg/kb, which
// ships its layouts. "google" is the input because repetition-adjacent
// replacement needs a doubled letter to work on.
func TestKeyboardAlgorithmsProduceVariants(t *testing.T) {
	for _, id := range []string{"aci", "acs", "rar"} {
		_, _, d, o := run(t, only(t, id), variant.TypeDomain, "google.com")
		if o.Status != graph.StatusOK || len(d.Edges) == 0 {
			t.Errorf("%s produced nothing (%v); keyboard data missing?", id, o.Status)
		}
	}
}

// variant.Language algorithms run over whatever variant.Language they are given.
func TestLanguageAlgorithmsProduceVariants(t *testing.T) {
	o := variant.Options{Languages: []variant.Language{testLang{}}}
	for _, id := range []string{"vs", "gi", "gr", "hr", "hs", "cm"} {
		ops, err := Select(o, id)
		if err != nil {
			t.Fatalf("%s: %v", id, err)
		}
		if len(ops) != 1 {
			t.Fatalf("%s selected %d operators, want 1", id, len(ops))
		}
		_, _, d, out := run(t, ops[0], variant.TypeDomain, "google.com")
		if out.Status != graph.StatusOK || len(d.Edges) == 0 {
			t.Errorf("%s produced nothing (%v)", id, out.Status)
		}
	}
}
...
func TestEdgePropsCarryAlgorithmAndDistance(t *testing.T) {
	op := only(t, "co")
	g, seed, d, _ := run(t, op, variant.TypeDomain, "example.com")
	...
	for _, e := range g.Edges() {
		if e.Rel.Name() != variant.Rel {
			continue
		}
		if e.From != seed {
			t.Fatalf("edge runs from %s, want the origin %s", e.From, seed)
		}
		algoField, ok := e.Rel.Field(variant.PropAlgorithm)
		if !ok {
		...
		distField, ok := e.Rel.Field(variant.PropDistance)
		if !ok {
...
func TestDistanceIsLevenshtein(t *testing.T) {
	...
	_, _, d, _ := run(t, only(t, "co"), variant.TypeUsername, "alice")
	for _, ps := range d.Props {
		if ps.Field != variant.PropDistance {
			continue
		}
		if ps.Value.Num() != 1 {
	...
	_, _, d, _ = run(t, only(t, "cr"), variant.TypeUsername, "alice")
	for _, ps := range d.Props {
		if ps.Field == variant.PropDistance && ps.Value.Num() != 1 {
...
func TestEachPropNamesItsOwnEdge(t *testing.T) {
	_, _, d, _ := run(t, only(t, "co"), variant.TypeDomain, "example.com")
	seen := map[string]int{}
	for _, ps := range d.Props {
		if ps.Edge == nil {
		...
		if ps.Edge.Rel != variant.Rel {
			t.Fatalf("prop on relation %q, want %s", ps.Edge.Rel, variant.Rel)
		}
		seen[ps.Edge.To.Key]++
	}
	for k, n := range seen {
...
func TestDeltaIntroducesVariantsOnlyByTheirEdge(t *testing.T) {
	for _, op := range All(testOptions()) {
		_, _, d, _ := run(t, op, variant.TypeDomain, "example.com")
		if len(d.Nodes) != 0 {
...
func TestSameInputYieldsByteIdenticalDelta(t *testing.T) {
	...
	for _, op := range All(testOptions()) {
		var first string
		for i := 0; i < runs; i++ {
			_, _, d, _ := run(t, op, variant.TypeDomain, "one.two-three.example.com")
			got := dump(d)
			if i == 0 {
				first = got
				continue
...
func TestExpansionIsReproducible(t *testing.T) {
	expand := func() []string {
		g := graph.New(testRegistry(t))
		if _, err := g.Seed(variant.TypeDomain, "example.com"); err != nil {
			t.Fatalf("seed: %v", err)
		}
...
func TestVariantsAreTerminal(t *testing.T) {
	g := graph.New(testRegistry(t))
	seed, err := g.Seed(variant.TypeDomain, "example.com")
	if err != nil {
		t.Fatalf("seed: %v", err)
	}
...
func TestVariantOutsideTheClosureIsRejected(t *testing.T) {
	...
	g.Apply(by, seed, graph.Delta{Edges: []graph.EdgeRef{{
		From: graph.NodeRef{Type: variant.TypeDomain, Key: "example.com"},
		Rel:  "RESOLVES_TO",
	...
	g.Apply(by, ipID, graph.Delta{Edges: []graph.EdgeRef{{
		...
		Rel:  "PTR_TO",
		To:   graph.NodeRef{Type: variant.TypeDomain, Key: "neighbour.example.net"},
	}}})
...
func TestSplitDomain(t *testing.T) {
	for _, tc := range []struct{ in, prefix, name, suffix string }{
		...
		p, n, s := variant.SplitDomain(tc.in)
...
func TestBarePublicSuffixFallsBackToWholeKey(t *testing.T) {
	_, _, d, o := run(t, only(t, "co"), variant.TypeDomain, "com")
	if o.Status != graph.StatusOK || len(d.Edges) == 0 {
...
func TestNonDomainTypesAreVariedWhole(t *testing.T) {
	p := variant.DefaultSplit(variant.TypePackage, "acme.tool")
	if p.Core != "acme.tool" {
...
func TestPublicSuffixesDropMatchingMarkers(t *testing.T) {
	for _, s := range variant.PublicSuffixes() {
...
func TestComboJoinForms(t *testing.T) {
	got := cb.ComboSquatting([]string{"login"})("paypal")
	sort.Strings(got)
	want := []string{"login-paypal", "loginpaypal", "paypal-login", "paypallogin"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("join forms:\n got %q\nwant %q", got, want)
	}
}

// The keyword attaches to the registrable name, never the whole key. A
func TestComboPreservesSuffix(t *testing.T) {
	op := variant.New(cb.Spec([]string{"login"}), nil)
	_, _, d, _ := run(t, op, "domain", "paypal.com")
	keys := variantKeys(d)
	...
	for _, k := range keys {
		if !strings.HasSuffix(k, ".com") {
			t.Errorf("variant %q lost the suffix", k)
		}
		if strings.Contains(k, ".com-") || strings.HasSuffix(k, ".comlogin") {
			t.Errorf("variant %q attached the keyword past the registrable name", k)
		}
	}
}

// A keyword equal to the name is a doubling, which the repetition algorithms
// own. Emitting it here would attribute one shape to two algorithms.
func TestComboSkipsKeywordEqualToName(t *testing.T) {
	for _, v := range cb.ComboSquatting([]string{"paypal", "login"})("paypal") {
		if v == "paypal-paypal" || v == "paypalpaypal" {
			t.Fatalf("emitted self-combination %q", v)
		}
	}
}

// Output must not depend on the order keywords arrive in: the vocabulary is
func TestComboIsOrderIndependent(t *testing.T) {
	a := cb.ComboSquatting([]string{"pay", "login", "secure"})("brand")
	b := cb.ComboSquatting([]string{"secure", "pay", "login"})("brand")
	sort.Strings(a)
	sort.Strings(b)
	if !reflect.DeepEqual(a, b) {
		t.Fatalf("keyword order changed the output:\n %q\nvs %q", a, b)
	}
}

func TestComboDedupesKeywords(t *testing.T) {
	got := cb.ComboSquatting([]string{"login", "login", "login"})("brand")
	if len(got) != cb.JoinsPerKeyword {
		t.Fatalf("duplicate keywords produced %d variants, want %d", len(got), cb.JoinsPerKeyword)
	}
}

// An explicitly empty vocabulary silences the algorithm rather than falling
func TestComboEmptyVocabularyIsSilent(t *testing.T) {
	if got := cb.ComboSquatting(nil)("brand"); len(got) != 0 {
		t.Fatalf("empty vocabulary produced %d variants", len(got))
	}
}

func TestSynonymDatasetLoadsEveryLanguage(t *testing.T) {
	langs := datasets.SynonymLanguages()
	if len(langs) < 25 {
		t.Fatalf("only %d languages ship synonym sets: %q", len(langs), langs)
	}
	for _, l := range langs {
		if len(datasets.SynonymWords(l)) == 0 {
			t.Errorf("language %q has an empty vocabulary", l)
		}
	}
}

func TestSynonymWordsAreSortedAndUnique(t *testing.T) {
	words := datasets.SynonymWords("en")
	if len(words) == 0 {
		t.Fatal("no English vocabulary")
	}
	for i := 1; i < len(words); i++ {
		if words[i-1] >= words[i] {
			t.Fatalf("not sorted/unique at %d: %q then %q", i, words[i-1], words[i])
		}
	}
}

// The number that decides whether cb is safe to leave on by default. It is an
func TestComboVocabularySize(t *testing.T) {
	en := len(datasets.SynonymWords("en"))
	t.Logf("english vocabulary: %d keywords -> %d variants per name",
		en, en*cb.JoinsPerKeyword)

	all := map[string]bool{}
	for _, l := range datasets.SynonymLanguages() {
		for _, w := range datasets.SynonymWords(l) {
			all[w] = true
		}
	}
	t.Logf("all languages: %d keywords -> %d variants per name",
		len(all), len(all)*cb.JoinsPerKeyword)

	if en > 400 {
		t.Errorf("english vocabulary grew to %d keywords; cb emits %d variants "+
			"per name, which starves the frontier", en, en*cb.JoinsPerKeyword)
	}
}

// The default must be one language's vocabulary, not every registered
// language's. Frontier and NodeBudget both default to unbounded, so nothing
func TestDefaultVocabularyIsSingleLanguage(t *testing.T) {
	o := variant.Options{}.WithDefaults()

	want := datasets.SynonymWords(variant.DefaultComboLanguage)
	if !reflect.DeepEqual(o.Combos, want) {
		t.Fatalf("default vocabulary is %d keywords, want the %d of %q",
			len(o.Combos), len(want), variant.DefaultComboLanguage)
	}

	// Against the shipped dataset rather than the registered plugins: this
	// package's tests register English alone, so a plugin-based comparison
	// would pass trivially and stop catching the regression it exists for.
	all := map[string]bool{}
	for _, l := range datasets.SynonymLanguages() {
		for _, w := range datasets.SynonymWords(l) {
			all[w] = true
	...
	if len(o.Combos) >= len(all) {
		t.Fatalf("default vocabulary (%d) is not narrower than the full dataset (%d)",
			len(o.Combos), len(all))
	}
}

// An explicit empty slice must survive withDefaults, or a caller cannot turn
func TestExplicitEmptyCombosIsNotDefaulted(t *testing.T) {
	o := variant.Options{Combos: []string{}}.WithDefaults()
	if len(o.Combos) != 0 {
		t.Fatalf("explicit empty vocabulary was replaced with %d keywords", len(o.Combos))
	}
}
