...
func TestSpecDeclaresTheAlgorithm(t *testing.T) {
	s := vs.Spec(varianttest.Langs())
	if s.ID != "vs" {
		t.Errorf("ID = %q, want %q", s.ID, "vs")
	}
	if s.Title == "" {
...
// each vowel replaced by every other vowel.
func TestGeneratesKnownVariants(t *testing.T) {
	got := vs.Spec(varianttest.Langs()).Gen("ab")
	want := []string{"eb", "ib", "ob", "ub"}
...
func TestNeverReturnsItsInput(t *testing.T) {
	for _, in := range []string{"ab", "example", "example.com"} {
		for _, g := range vs.Spec(varianttest.Langs()).Gen(in) {
			if g == in {
...
func TestReturnsNoDuplicates(t *testing.T) {
	...
	for _, g := range vs.Spec(varianttest.Langs()).Gen("ab") {
		if seen[g] {
...
func TestEmptyInputIsQuiet(t *testing.T) {
	if got := vs.Spec(varianttest.Langs()).Gen(""); len(got) != 0 {
