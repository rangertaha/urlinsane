...
func TestNothingToDecomposeIsEmptyNotFailed(t *testing.T) {
	for _, tc := range []struct {
		...
		{TypePackage, "npm:lodash", "decompose.package", graph.StatusEmpty},
		{TypePackage, "npm:@acme/tool", "decompose.package", graph.StatusOK},
		{TypeDomain, "localhost", "decompose.domain", graph.StatusEmpty},
		{TypeDomain, "example.com", "decompose.domain", graph.StatusOK},
		{TypeEmail, "bob@example.com", "decompose.email", graph.StatusOK},
	} {
...
func TestRegisterMatchesDesign(t *testing.T) {
	...
	types := map[string]graph.Capability{
		TypeDomain:     graph.Nameable,
		TypeUsername:   graph.Nameable,
		TypePackage:    graph.Nameable,
		TypeRepo:       graph.Nameable,
		TypeEmail:      graph.Nameable,
		TypeTLD:        graph.Observed,
		TypeIP:         graph.Observed,
		TypeASN:        graph.Observed,
		TypeRegistrant: graph.Observed,
		TypePlatform:   graph.Observed,
	}
	...
	rels := map[string]graph.Class{
		RelLocalPart:    graph.Structural,
		RelDomainOf:     graph.Structural,
		RelTLDOf:        graph.Structural,
		RelOwner:        graph.Structural,
		RelHostedOn:     graph.Structural,
		RelVariantOf:    graph.Variant,
		RelManifest:     graph.Observation,
		RelResolvesTo:   graph.Observation,
		RelNS:           graph.Observation,
		RelMX:           graph.Observation,
		RelPTRTo:        graph.Observation,
		RelInASN:        graph.Observation,
		RelRegisteredBy: graph.Observation,
		RelExistsOn:     graph.Observation,
	}
	...
	variant, _ := r.Rel(RelVariantOf)
	for _, f := range []string{FieldAlgorithm, FieldDistance} {
		if _, ok := variant.Field(f); !ok {
			t.Errorf("%s has no %q field", RelVariantOf, f)
		}
	}
}
...
func TestCanonicalization(t *testing.T) {
	for _, tc := range []struct {
		...
		{typ: TypeDomain, raw: "Example.COM.", want: "example.com"},
		{typ: TypeDomain, raw: " münchen.de ", want: "xn--mnchen-3ya.de"},
		{typ: TypeDomain, raw: "xn--mnchen-3ya.de", want: "xn--mnchen-3ya.de"},
		...
		{typ: TypeDomain, raw: "-exampl-e.com", want: "-exampl-e.com"},
		{typ: TypeDomain, raw: "   ", wantErr: true},
		{typ: TypeDomain, raw: "http://example.com/x", wantErr: true},

		{typ: TypeEmail, raw: "Bob@Example.COM", want: "Bob@example.com"},
		{typ: TypeEmail, raw: "bob", wantErr: true},
		{typ: TypeEmail, raw: "@example.com", wantErr: true},

		{typ: TypeUsername, raw: "Acme", want: "acme"},
		{typ: TypeUsername, raw: "", wantErr: true},

		{typ: TypeRepo, raw: "github.com/Acme/Tool", want: "github.com/acme/tool"},
		{typ: TypeRepo, raw: "https://GitHub.com/acme/tool.git", want: "github.com/acme/tool"},
		{typ: TypeRepo, raw: "git@github.com:acme/tool.git", want: "github.com/acme/tool"},
		{typ: TypeRepo, raw: "https://github.com/acme/tool/tree/main", want: "github.com/acme/tool"},
		{typ: TypeRepo, raw: "github.com/acme", wantErr: true},

		{typ: TypePackage, raw: "npm:Lodash", want: "npm:lodash"},
		{typ: TypePackage, raw: "NPM:@Acme/Tool", want: "npm:@acme/tool"},
		{typ: TypePackage, raw: "pypi:Foo_Bar.baz", want: "pypi:foo-bar-baz"},
		{typ: TypePackage, raw: "pypi:foo--bar", want: "pypi:foo-bar"},
		// Unqualified: npm's lodash and PyPI's lodash are different entities.
		{typ: TypePackage, raw: "lodash", wantErr: true},

		{typ: TypeTLD, raw: ".COM", want: "com"},
		{typ: TypeIP, raw: " 1.2.3.4 ", want: "1.2.3.4"},
		{typ: TypeIP, raw: "::ffff:1.2.3.4", want: "1.2.3.4"},
		{typ: TypeIP, raw: "2001:0DB8:0000::1", want: "2001:db8::1"},
		{typ: TypeIP, raw: "example.com", wantErr: true},
		{typ: TypeASN, raw: "15169", want: "AS15169"},
		{typ: TypeASN, raw: "as015169", want: "AS15169"},
		{typ: TypeASN, raw: "ASN15169", want: "AS15169"},
		{typ: TypeASN, raw: "AS-15169", wantErr: true},
		{typ: TypeRegistrant, raw: "  Google   LLC ", want: "google llc"},
		{typ: TypeRegistrant, raw: " ", wantErr: true},
		{typ: TypePlatform, raw: "GitHub.com.", want: "github.com"},
	} {
